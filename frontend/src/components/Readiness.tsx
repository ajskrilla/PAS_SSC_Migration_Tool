...
/**
 * Readiness page (merges Setup + Verification).
 *
 * Two sections: PRE-MIGRATION prerequisites (interactive checklist + procedures) and
 * POST-MIGRATION verification checks. State only - no persistence, resets on reload.
...
const VERIFY_ITEMS: Item[] = [
  {
    id: "v-counts",
    label: "Source and target counts reconcile",
    detail: (
      <>
        Use the reconciliation view on the Overview page. Matched counts should account for every
        source item; investigate any unexpected source-only entries.
      </>
    ),
  },
  {
    id: "v-passwords",
    label: "Account password fields populated in Secret Server",
    detail: (
      <>
        Spot-check a few migrated accounts in Secret Server and confirm the password field is filled,
        not blank. Blank fields usually mean a template slug mismatch.
      </>
    ),
  },
  {
    id: "v-files",
    label: "File secrets have their attachments (byte-fidelity)",
    detail: (
      <>
        Confirm migrated file secrets show an attachment, and for important files download one and
        compare it to the source to verify the bytes match.
      </>
    ),
  },
  {
    id: "v-multiplexed",
    label: "Multiplexed accounts handled manually",
    detail: (
      <>
        Multiplexed accounts have no migration path and are skipped by the tool. Confirm any flagged
        in inventory have been handled separately.
      </>
    ),
  },
  {
    id: "v-stranded",
    label: "No accounts left unmanaged-but-not-migrated",
    detail: (
      <>
        If a checkout failed after unmanage, an account can be left unmanaged in PAS without landing
        in Secret Server. Check the logs for this flag and remediate any.
      </>
    ),
  },
];
    ...
    <div>
      ...
      <div className="panel" style={{ marginBottom: 16 }}>
        <p className="muted" style={{ marginTop: 0 }}>
          This page covers both ends of a migration: the <strong>prerequisites</strong> to complete
          before you start, and the <strong>verification</strong> checks to run after. Some items are
          typically performed by a tenant administrator &mdash; hand those to your Delinea admin if needed.
        </p>
        <div className="setup-progress">
          ...
          <span className={`muted ${allDone ? "setup-done" : ""}`}>
            Prerequisites {doneCount} / {ITEMS.length} complete{allDone ? " \u2713" : ""}
          </span>
        </div>
      </div>

      <div className="phase-no" style={{ fontSize: 13, marginBottom: 8 }}>PRE-MIGRATION</div>
      ...
      <div className="phase-no" style={{ fontSize: 13, margin: "24px 0 8px" }}>POST-MIGRATION</div>
      <section className="panel">
        <div className="phase-no">VERIFICATION CHECKS</div>
        <h2 style={{ marginTop: 0 }}>After migrating, confirm the results</h2>
        <p className="muted">
          Run these checks once a migration completes. They confirm the data landed intact and that
          no accounts were left in a half-migrated state.
        </p>
        <div className="setup-list">
          {VERIFY_ITEMS.map((it) => (
            <label key={it.id} className={`setup-row ${checked[it.id] ? "checked" : ""}`}>
        ...
        <p className="muted" style={{ fontSize: 12, marginTop: 12, marginBottom: 0 }}>
          The Assistant can run several of these checks for you automatically &mdash; ask it to verify
          permissions, UVA mode, or migration progress.
        </p>
      </section>
    </div>
  );
